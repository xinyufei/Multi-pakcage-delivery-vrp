...
def SolveByCG(instance_index):
    ...
    y_pre = np.zeros((num_product*(num_node-1), num_product*(num_node-1)))
    cost_pre_comp = np.zeros((num_product*(num_node-1), num_product))
    cost_pre = np.zeros(num_product*(num_node-1))
    ...
    for l in range(num_product):
        ...
        while (i < num_node):
            if cur_p + Demand_pre[l][i] <= Q_product[l]:
                if Demand_pre[l][i] != 0:
                    y_pre[(i-1)*num_product+l][(v-1)*num_product+l] = Demand_pre[l][i]
                    pass_i.append(i)
                """ if Demand_pre[l][i] == 0:
                    ...
                    # v = v+1
                i = i+1
                cur_p = sum(y_pre[(j-1)*num_product+l][(v-1)*num_product+l] for j in pass_i)
            else:
                if cur_p == Q_product[l]:
                    # for k in range(start_i, i):
                    cost_pre_comp[(v-1)*num_product+l,l] = sum(cost[pass_i[j], pass_i[j+1]] for j in range(len(pass_i)-1)) + cost[0,pass_i[0]] + cost[pass_i[len(pass_i)-1],0]
                    cur_p = 0
                    # start_i = i
                    v = v+1
                    num_v += 1
                    ...
                    y_pre[(i-1)*num_product+l][(v-1)*num_product+l] = Q_product[l] - cur_p
    ...
    for i in range(1,num_node):
        for j in range(num_product):
            ...
            f[(i-1)*num_product+j] = master.addVar(obj = cost_pre[(i-1)*num_product+j], lb = 0, ub = 1, vtype = gb.GRB.CONTINUOUS,  name="route[%d]"%((i-1)*num_product+j))
    ...
    for i in range(1,num_node):
        for j in range(num_product):
            # demand_cons[(i-1)*num_product+j] = master.addConstr(Demand[j][i]*f[(i-1)*num_product+j] >= Demand[j][i], name="demand[%d]"%((i-1)*num_product+j))
    ...
    f[(num_node-1)*num_product] = master.addVar(obj = cost[0,1]+cost[1,2]+cost[2,0], lb = 0, ub = 1, vtype = gb.GRB.CONTINUOUS, name="route[%d]"%((num_node-1)*num_product))
    prob = master.addVar(lb=0, ub=1)
    vehicle_cons = master.addConstr(gb.quicksum(f[i] for i in range((num_node-1)*num_product)) <= num_vehicle)
    master.update()

    num_var = (num_node-1)*num_product
    route = {}
    cost_value = {}
    start = time.time()
    while 1:
